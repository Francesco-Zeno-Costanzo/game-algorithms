...
def minimax(stato, depth, giocatore):
    ...
    #punteggi da massimizzare e minimazzare
    if giocatore == Engine:
        best = [-1, -1, -infinity]
    else:
        best = [-1, -1, +infinity]
    ...
    #loop su tutte le caselle libere
    for cella in libero(stato):
        #tentativo mossa
        x, y = cella[0], cella[1]
        stato[x][y] = giocatore
        #probabile mossa avversaria( si ricorda che
        #giocatore è una variabile che può valere +-1) e
        #chiamata ricorsiva della funzione per creare ogni
        #partita nell'albero
        punteggio = minimax(stato, depth - 1, -giocatore)
        stato[x][y] = 0
        punteggio[0], punteggio[1] = x, y

        #massimmizzo per il giocatore Engine
        if giocatore == Engine:
            if punteggio[2] > best[2]:
                best = punteggio
        #minimizzo per l'altro giocatore
        else:
            if punteggio[2] < best[2]:
                best = punteggio
